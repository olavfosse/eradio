...
** VLC
eradio requires that vlc is in =$PATH= and capable of playing Internet radio. On Linux and most other Unix-like system this requirement can be satisfied by simply installing the vlc package from your distribution's repository.

On MacOS this can be done by installing VLC from [[https://www.videolan.org/]], writing the following to =/usr/local/bin/vlc= and making it executable with =chmod +x /usr/local/bin/vlc=.
#+BEGIN_SRC sh
#!/bin/sh                                                                       
exec '/Applications/VLC.app/Contents/MacOS/VLC' "$@"
#+END_SRC
*WARNING* Symlinking =/usr/local/bin/vlc= to =/Applications/VLC.app/Contents/MacOS/VLC= does not work.
